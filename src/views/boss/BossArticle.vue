      ...
      <div class="author-info">
        ...
        <div class="author-info-detail">
          ...
          <div class="author-info-badge">
            共
            <mt-badge type="primary" size="small">
...
export default {
  data() {
    return {
      id: this.$route.query.id, // 从路由地址中获取id参数
      article: null, // 文章对象
    };
  },
  mounted() {
    // 发送http请求, 加载相应id的详情数据:  /detail
    this.axios.get(`/bossdetail?id=${this.id}`).then((res) => {
      console.log("加载详情数据, id:" + this.id, res);
      this.article = res.data.result; // 将文章对象存入data.article
    });
  },
};
</script>
